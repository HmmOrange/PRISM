...
class PromptTemplateManager(BaseModel):
    ...
    def initialize_templates(self) -> Self:
        self.templates_dir = os.path.join(ROOT_PATH, "prism", "prompts", "templates")
    ...
    def _load_templates(self) -> None:
        ...
        self._load_templates_from_dir(self.templates_dir, "prism.prompts.templates")
    ...
    def _load_templates_from_dir(self, templates_dir: str, templates_module: str) -> None:
        """
        ...
        if not os.path.exists(templates_dir):
            if templates_dir == self.templates_dir:
                ...
                logger.warning(f"Custom templates directory '{templates_dir}' does not exist, skipping.")
                return
        ...
        for filename in os.listdir(templates_dir):
            if filename.endswith(".py") and filename != "__init__.py":
                ...
                if script_name in self.templates:
                    logger.debug(f"Template '{script_name}' already exists, skipping from '{templates_dir}'")
                    continue
