...
from prism.utils.report import NotebookReporter
...
class NbCellRecord:
    ...
    def __init__(self, code: str, language: str, output: str, success: bool, timestamp: Optional[datetime] = None, metadata: dict = {}):
        self.code = code
    ...
    def format_history(self, include_output: bool = True, max_code_len: int = 1000, max_output_len: int = 500):
...
class NbCodeWorkspace(ContextMixin, BaseModel):
    ...
    def add_to_history(self, code: str, language: str, output: str, success: bool, metadata: dict = {}):
        record = NbCellRecord(code=code, language=language, output=output, success=success)
    ...
    def get_history(self, last_n: Optional[int] = None, include_output: bool=True,
                    filter_success: Optional[bool] = None, filter_language: Optional[str] = None, filter_task_ids: list = []):
        # Filter records
        filtered_records = self.history
        
        if filter_success is not None:
            filtered_records = [r for r in filtered_records if r.success == filter_success]
            
        if filter_language:
            filtered_records = [r for r in filtered_records if r.language == filter_language]
        
        if len(filter_task_ids) > 0:
            filtered_records = [r for r in filtered_records if r.metadata["task_id"] in filter_task_ids]
...
"""
        ...
        for i, output in enumerate(outputs):
            ...
            if output_dict["output_type"] == "stream" and not any(
                ...
                if "image/png" in output_dict["data"]:
                    interaction_type = "ipython" if self.interaction == "ipython" else None
                    self.show_bytes_figure(output_dict["data"]["image/png"], interaction_type)
                else:
                    logger.info(
    ...
    async def safe_run(self, code: str, language: Literal["python", "markdown"] = "python", metadata: dict = {}) -> Tuple[str, bool]:
        """
        ...
        try:
            # Run code
            output, success = await self.run(code, language)
            output = output.replace(",from .autonotebook import tqdm as notebook_tqdm", "")
            if success:
                # self.add_to_history(code, language, output, success, metadata)
                print(output)
