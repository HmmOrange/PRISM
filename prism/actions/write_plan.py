...
from prism.schema.plan import Plan, Task
from prism.schema.task_type import TaskType
    ...
    if len(tasks) == 1 or tasks[0].dependent_task_ids:
        ...
        if current_plan.has_task_id(tasks[0].task_id):
            ...
            current_plan.replace_task(
                tasks[0].task_id, tasks[0].dependent_task_ids, tasks[0].instruction, tasks[0].assignee, task_type=tasks[0].task_type, expected_output=""
            )
        else:
            # append one task
            current_plan.append_task(
                tasks[0].task_id, tasks[0].dependent_task_ids, tasks[0].instruction, tasks[0].assignee, task_type=tasks[0].task_type
            )
