...
from prism.utils.repair_llm_raw_output import extract_state_value_from_output
from prism.strategy.planner import Planner
from prism.schema.plan import Task, TaskResult
from prism.actions.user_requirement import UserRequirement
from prism.actions.action import Action
from prism.actions.action_output import ActionOutput
from prism.schema.context_mixin import ContextMixin
from prism.base.base_env import BaseEnvironment
from prism.schema.message import Message, MessageQueue, AIMessage
from prism.schema.memory import Memory
from prism.base.base_role import BaseRole
from prism.environment.nb_workspace import NbCodeWorkspace
...
class AgentContext(BaseModel):
    ...
    env: Optional[BaseEnvironment] = Field(default=None, exclude=True) # # avoid circular import
...
class BaseAgent(BaseRole, ContextMixin, BaseModel):
    ...
    __hash__ = object.__hash__  # support BaseAgent as hashable type in Environment.members
    ...
    def _check_actions(self):
        ...
        if self.actions:
            self.set_actions(list(self.actions))  # Ép kiểu sang list[Action | type[Action]] để tránh lỗi kiểu
        return self
    ...
    def set_env(self, env: BaseEnvironment):
        ...
        if env:
            ...
            self.set_actions(list(self.actions))  # reset actions to update llm and prefix
    ...
    def set_addresses(self, addresses: Set[str]):
        ...
        if self.ac.env:  # According to the routing feature plan in Chapter 2.2.3.2 of RFC 113
    ...
    async def _observe(self) -> bool:
        ...
        self.ac.news = [
            n for n in news if (n.cause_by in self.ac.watch or self.name in n.send_to) and n not in old_messages
        ]
        ...
        self.latest_observed_msg = self.ac.news[-1] if self.ac.news else None  # record the latest observed msg
    ...
    async def _plan_and_act(self) -> Message:
        ...
        if not self.planner.plan.goal:
            ...
            goal = self._init_action.memory.get()[-1].content  # retreive latest user requirement
    ...
    async def react(self) -> Message:
        if self.ac.react_mode == RoleReactMode.REACT or self.ac.react_mode == RoleReactMode.BY_ORDER:
