...
def describe_dict(data: Dict[str, Any], indent: int = 0) -> str:
    result = "{\n"
    pad = "  " * (indent + 1)

    for key, value in data.items():
        if isinstance(value, dict):
            # Nếu value là dict → đi sâu hơn
            result += f'{pad}"{key}": A dict with key {describe_dict(value, indent + 1)}\n'
        elif isinstance(value, list):
            if len(value) == 0:
                result += f'{pad}"{key}": List (empty)\n'
            else:
                # Lấy kiểu của phần tử đầu tiên
                elem_type = type(value[0]).__name__
                result += f'{pad}"{key}": List of {elem_type}\n'
        else:
            # Các kiểu cơ bản
            value_type = type(value).__name__
            result += f'{pad}"{key}": {value_type}\n'
