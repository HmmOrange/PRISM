...
#             return total_params
#     # return float('inf')
#     # Assume every heavy model will have params, light model may not have params
#     return 0

# def get_model_information(model_id: str) -> Optional[dict]:
...
#     if num_parameters and num_parameters > MAX_PARAMETERS:
#         return None
#     try:
...
# # Load models from main huggingface_models.jsonl and external_huggingface_models.jsonl
# model_info_paths = [f"{ROOT_PATH}/data/huggingface_models.jsonl", f"{ROOT_PATH}/data/external_huggingface_models.jsonl"]
# for model_info_path in model_info_paths:
#     with open(model_info_path, "r", encoding="utf-8") as f:
#         for line in f.readlines():
#             info = json.loads(line)
#             pipes[info["id"]] = load_model(info)
...
#         print(f"[ warning ] Model {model_id} not found in huggingface_models.jsonl, fetching from HuggingFace")
#         model_information = get_model_information(model_id)
#         if not model_information:
#             return None
#         with open(f"{ROOT_PATH}/data/external_huggingface_models.jsonl", "a", encoding="utf-8") as file:
#             file.write(json.dumps(model_information, ensure_ascii=False) + "\n")
#         pipes[model_id] = load_model(model_information)
...
#     try:
#         tokenizer = AutoTokenizer.from_pretrained(pipe["model_path"])
#         pipes[model_id]["max_model_len"] = tokenizer.model_max_length
#         del tokenizer
#         print(f"[ success ] Get model {model_id} with max model length {pipes[model_id]['max_model_len']}")
#     except Exception as e:
#         print(f"[ warning ] Cannot get max model length for model {model_id}: {e}")
#         pipes[model_id]["max_model_len"] = None
#     pipe["model"] = model
...
#         text = data["text"]
#         if pipe["max_model_len"] is not None:
#             text = text[:pipe["max_model_len"]]
#         r = model(text)
