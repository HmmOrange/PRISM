...
class masPipeline(Pipeline, BaseModel):
    ...
    async def __call__(self, task_description: str, data: dict) -> Tuple[str, float]:
        ...
        workflow_path = os.path.join(
            self.default_path, "workflows", self.level, self.task_name
        )
        ...
        for i in range(1, max_iterations + 1):
            cost_manager.reset()
            start_time = time()
            experiences = self.experience_utils.load_all_experiences(
                workflow_path, i - 1
            )
            formatted_experiences = self.experience_utils.format_experience(experiences)

            sketch, graph = await self.workflow_utils.generate_mas(
            ...
            self.workflow_utils.write_workflow_file(
                graph, i, workflow_path, is_solution=False
            )
            self.workflow_utils.write_mermaid_file(sketch, i, workflow_path)

            sketch = self.workflow_utils.read_mermaid_file(i, workflow_path)
            graph = self.workflow_utils.read_workflow_file(
                i, workflow_path, is_solution=False
            )

            workflow_instance = self.workflow_utils.load_workflow(i, workflow_path)
            workflow_instance = workflow_instance(
                problem=task_description, data=validation_data_5
            )
            code = await workflow_instance()
            self.workflow_utils.write_workflow_file(
                code, i, workflow_path, is_solution=True
            )

            code = self.workflow_utils.read_workflow_file(
                i, workflow_path, is_solution=True
            )
            ...
            review = await self.workflow_utils.review_mas(
                task_description=task_description,
                model_type_description=model_type_description,
                sketch=sketch,
                code=code,
            ...
            self.experience_utils.update_experience(workflow_path, i, experience)
            if self.convergence_utils.check_convergence(workflow_path, i):
                break

        best_round = await self.data_utils.select_best_round(
            workflow_path, max_iterations, user_requirement=task_description
        )
