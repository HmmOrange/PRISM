...
def update_total_cost(task_name: str, usage):
    try:
        with open("data/fewshot_cost.json", "r", encoding="utf-8") as f:
            data = json.load(f)
    except:
        data = {{}}
    ...
    with open("data/fewshot_cost.json", "w", encoding="utf-8") as f:
        json.dump(data, f)
...
class Workflow:
    async def __call__(self, data: dict) -> str:
        ...
        usr_msg = {{
            ...
            "content": [{{
                "type": "text",
                "text": FEW_SHOT_PROMPT.format(description=task_description, input_text=text_data)
            }}]
        }}
