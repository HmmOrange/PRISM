    ...
    for current_dir, _, files in os.walk(path):
        for file in files:
            ...
            if file_extension in extensions['image']:
                if "image_paths" not in result:
                    result['image_paths'] = []
                result['image_paths'].append(file_path.replace("\\", "/"))
            elif file_extension in extensions['video']:
                if "video_paths" not in result:
                    result['video_paths'] = []
                result['video_paths'].append(file_path.replace("\\", "/"))
            elif file_extension in extensions['audio']:
                if "audio_paths" not in result:
                    result['audio_paths'] = []
                result['audio_paths'].append(file_path.replace("\\", "/"))
            elif file == "record.json":
                if "text_data" not in result:
                    result['text_data'] = {}
...
def load_data(path: str) -> list:
    ...
    inputs = []
    label_file = load_label(path)
    label_file = label_file.set_index("id").to_dict(orient="index")
    for f in os.listdir(inputs_path):
        if os.path.isdir(os.path.join(inputs_path, f)):
            result = load_files_in_folder(os.path.join(inputs_path, f))
            result["label"] = label_file[int(f)]["label"]
            result["input_id"] = f
...
def get_available_datasets() -> Dict[str, Dict[str, str]]:
    ...
    return {
        ...
        "results": {
            "url": "https://drive.google.com/uc?export=download&id=1Mydxd0GOfCJEZZBEiZLU8FNNJCnYYVxE",
            "filename": "results.tar.gz",
            "extract_path": "results",
        },
        "workflows": {
            "url": "https://drive.google.com/uc?export=download&id=1yHv7lQ5u3xOC-YoVoq7zVta7GdH9zuTT",
            "filename": "results_workflows.tar.gz",
            "extract_path": "results_workflows",
        }
    }
